...
public class Solver {
    ...
    private static void setup(String[] input) {
        ...
        for (int i = 0; i < input.length; i++)
            puzzle[i] = input[i].split(" ");  //Metode split per dividir l'string i posar a les columnes
        ...
        board = new int[nRows + 2][nCols + 2];  //Coloca tot el taulell a -1
        for (int[] row : board)
            for (int c = 0; c < nCols + 2; c++)
                row[c] = -1;
 
        for (int r = 0; r < nRows; r++) {
            String[] row = puzzle[r];  //row conte la fila del puzzle
        ...
        Collections.sort(list);  //Aqui els nombres donats i ordenats en una llista
        given = new int[list.size()];
        for (int i = 0; i < given.length; i++)
            given[i] = list.get(i);
    }
 
    private static boolean solve(int r, int c, int n, int next) { //n=1, next = 0 1st iteration
